    ...
    <div className="animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          {/* Course intro card */}
          <div className="rounded-lg border p-6 shadow-sm animate-in fade-in duration-700 delay-[200ms]">
          ...
          {/* Course outline card */}
          <div className="rounded-lg border p-6 shadow-sm animate-in fade-in duration-700 delay-[400ms]">
            <Skeleton className="h-6 w-40 mb-4 skeleton-wave" />
            <Skeleton className="h-4 w-48 mb-6 skeleton-wave" />
            
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="space-y-3 animate-in fade-in duration-500" style={{animationDelay: `${400 + i * 150}ms`}}>
                  ...
                  <div className="pl-4 space-y-3">
                    {[1, 2, 3].map((j) => (
                      <div key={j} className="flex justify-between items-center animate-in fade-in duration-300" style={{animationDelay: `${500 + (i + j) * 100}ms`}}>
          ...
          {/* Course attachments */}
          <div className="rounded-lg border p-6 shadow-sm animate-in fade-in duration-700 delay-[600ms]">
            <Skeleton className="h-6 w-40 mb-4 skeleton-wave" />
            <div className="space-y-3">
              {[1, 2].map((i) => (
                <div key={i} className="flex justify-between items-center border rounded-lg p-4 animate-in fade-in duration-500" style={{animationDelay: `${700 + i * 150}ms`}}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-700 delay-[800ms]">
            {[1, 2, 3].map((i) => (
              <div key={i} className="rounded-lg border p-6 shadow-sm" style={{animationDelay: `${900 + i * 100}ms`}}>
                <Skeleton className="h-6 w-24 mb-4 skeleton-wave" />
                <div className="space-y-3">
                  {[1, 2, 3, 4].map((j) => (
                    <div key={j} className="flex items-center gap-2 animate-in fade-in duration-300" style={{animationDelay: `${1000 + (i + j) * 100}ms`}}>
