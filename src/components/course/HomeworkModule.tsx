...
export const HomeworkModule = ({ courseId, lectureId, onHomeworkSubmit }: HomeworkModuleProps) => {
  ...
  // 作业查询 - 确保使用数字类型的课程ID
  const { data: homework, isLoading: isLoadingHomework, refetch: refetchHomework } = useQuery({
    ...
    queryFn: async () => {
      ...
      return handleHomeworkQueryError(data, error);
    },
  ...
  }, [homework, numericCourseId, lectureId, user?.id, refetchHomework, isCreatingDefaultHomework, courseId]);
  ...
  }, [homework, submissions, allHomeworkCompleted, onHomeworkSubmit]);
